 ...
 *
 *****************************************************************************/
#define _RTL8733BU_OPS_C_

#include <drv_types.h>			/* PADAPTER, basic_types.h and etc. */
#include <hal_intf.h>			/* struct hal_ops */
#include <hal_data.h>			/* HAL_DATA_TYPE */
#include "../../hal_halmac.h"		/* register define */
#include "../rtl8733b.h"		/* 8733b hal common define. rtl8733bu_init_default_value ...*/
#include "rtl8733bu.h"			/* 8733bu functions */

#ifdef CONFIG_SUPPORT_USB_INT
static void rtl8733bu_interrupt_handler(PADAPTER padapter, u16 pkt_len, u8 *pbuf)
{	
	}
#endif /* CONFIG_SUPPORT_USB_INT */

void rtl8733bu_set_hw_type(struct dvobj_priv *pdvobj)
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8733BU;
...
static u8 sethwreg(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_SET_RPWM:
#ifdef CONFIG_LPS_LCLK
		{
			...
			* BIT0 value - 1: 32k(LCLK), 0: Normal Clock
			* BIT4 value - 1: Power Gating
			...
			*/
			...
			if ((ps_state & BIT(0)) && (LPS_PG == pwrpriv->lps_level))
				ps_state |= BIT(4);
			#endif
			rtw_write8(padapter, REG_USB_HRPWM_8733B, ps_state);
		}
#endif
		break;
	case HW_VAR_AMPDU_MAX_TIME:
		rtw_write8(padapter, REG_AMPDU_MAX_TIME_8733B, 0x70);
		break;
	...
	default:
		ret = rtl8733b_sethwreg(padapter, variable, val);
		break;
	}
...
static void gethwreg(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_CPWM:
#ifdef CONFIG_LPS_LCLK
		*val = rtw_read8(padapter, REG_USB_HCPWM_8733B);
		/* RTW_INFO("##### REG_USB_HCPWM(0x%02x) = 0x%02x #####\n", REG_USB_HCPWM_8733B, *val); */
#endif /* CONFIG_LPS_LCLK */
		break;
	case HW_VAR_RPWM_TOG:
#ifdef CONFIG_LPS_LCLK
		*val = rtw_read8(padapter, REG_USB_HRPWM_8733B);
		*val &= BIT_TOGGLE_8733B;
#endif /* CONFIG_LPS_LCLK */
		break;

	default:
		rtl8733b_gethwreg(padapter, variable, val);
		break;
	}
...
static u8 sethaldefvar(PADAPTER padapter, HAL_DEF_VARIABLE eVariable, void *pValue)
{
	...
	switch (eVariable) {
	default:
		rtl8733b_sethaldefvar(padapter, eVariable, pValue);
		break;
	}
...
static u8 gethaldefvar(PADAPTER	padapter, HAL_DEF_VARIABLE eVariable, void *pValue)
{
	...
	case HW_VAR_MAX_RX_AMPDU_FACTOR:
		*(HT_CAP_AMPDU_FACTOR *)pValue = MAX_AMPDU_FACTOR_64K;
		break;
	default:
		bResult = rtl8733b_gethaldefvar(padapter, eVariable, pValue);
		break;
	}
...
static void rtl8733bu_init_default_value(PADAPTER	padapter)
{
	rtl8733b_init_default_value(padapter);
}

static u8 rtl8733bu_ps_func(PADAPTER padapter, HAL_INTF_PS_FUNC efunc_id, u8 *val)
{
	u8 bResult = _TRUE;
...
static u8 read_adapter_info(PADAPTER padapter)
{
	 ...
	 * 1. Read Efuse/EEPROM to initialize
	 */
	if (rtl8733b_read_efuse(padapter) != _SUCCESS)
		goto exit;
...
void rtl8733bu_set_hal_ops(PADAPTER padapter)
{
	struct hal_ops *ops;
	int err;

	err = rtl8733bu_halmac_init_adapter(padapter);
	if (err) {
		RTW_INFO("%s: [ERROR]HALMAC initialize FAIL!\n", __func__);
		return;
	}

	rtl8733b_set_hal_ops(padapter);

	ops = &padapter->hal_func;

	ops->hal_init = rtl8733bu_init;
	ops->hal_deinit = rtl8733bu_deinit;

	ops->inirp_init = rtl8733bu_inirp_init;
	ops->inirp_deinit = rtl8733bu_inirp_deinit;

	ops->init_xmit_priv = rtl8733bu_init_xmit_priv;
	ops->free_xmit_priv = rtl8733bu_free_xmit_priv;

	ops->init_recv_priv = rtl8733bu_init_recv_priv;
	ops->free_recv_priv = rtl8733bu_free_recv_priv;
#ifdef CONFIG_RTW_SW_LED
	ops->InitSwLeds = rtl8733bu_initswleds;
	ops->DeInitSwLeds = rtl8733bu_deinitswleds;
#endif

	ops->init_default_value = rtl8733bu_init_default_value;
	ops->intf_chip_configure = rtl8733bu_interface_configure;
	...
	ops->hal_xmit = rtl8733bu_hal_xmit;
	ops->mgnt_xmit = rtl8733bu_mgnt_xmit;
#ifdef CONFIG_RTW_MGMT_QUEUE
	ops->hal_mgmt_xmitframe_enqueue = rtl8733bu_hal_mgmt_xmitframe_enqueue;
#endif
	ops->hal_xmitframe_enqueue = rtl8733bu_hal_xmitframe_enqueue;

#ifdef CONFIG_HOSTAPD_MLME
	ops->hostap_mgnt_xmit_entry = rtl8733bu_hostap_mgnt_xmit_entry;
#endif
	ops->interface_ps_func = rtl8733bu_ps_func;
#ifdef CONFIG_XMIT_THREAD_MODE
	ops->xmit_thread_handler = rtl8733bu_xmit_buf_handler;
#endif
#ifdef CONFIG_SUPPORT_USB_INT
	ops->interrupt_handler = rtl8733bu_interrupt_handler;
#endif
}
